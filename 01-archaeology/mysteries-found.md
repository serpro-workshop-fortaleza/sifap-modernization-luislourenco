...
| `BONUS` | Por que existem **quatro** implementações distintas de validação de CPF por módulo 11? | `CCVALCPF.NSC:39-130`, `CADBENEF.NSP:344-413`, `VALBENEF.NSN:196-276`, `VALDOCS.NSP:137-199` | A "fonte única de verdade" declarada em `SUBVALCP.NSN:8` não é única; o Java precisa escolher uma. Só `SUBVALCP.NSN:94` e `CADDEPEN.NSP:230` usam o copycode | Não confirmada: consolidação iniciada em 2011 e nunca concluída | Coordenação de Benefícios | aberta |
| `BONUS` | Por que o cabeçalho do copycode padrão avisa que as cópias "NÃO SÃO EQUIVALENTES" e o ticket 6620/2011 continua aberto há 15 anos? | `CCVALCPF.NSC:32-37` | A divergência é conhecida e documentada pela própria equipe legada, mas nunca resolvida; a migração herda a decisão pendente | Não confirmada: substituir as cópias mudaria quem passa na validação, e o impacto nunca foi medido | Coordenação de Benefícios | aberta |
| `BONUS` | Por que a rotina padrão rejeita CPF com todos os dígitos iguais, mas `CADBENEF` e `VALDOCS` não têm essa verificação? | `CCVALCPF.NSC:79-90` contra `CADBENEF.NSP:344-413` e `VALDOCS.NSP:137-199` | `11111111111` é aceito no cadastro de beneficiário e na validação de documentos, e rejeitado por `SUBVALCP`; amplia o `SIFAP-M-14` de duas para quatro variantes | Não confirmada: a verificação de 17/05/2005 (`CCVALCPF.NSC:6`) só foi aplicada ao copycode, não às cópias inline | Coordenação de Benefícios | aberta |
| `BONUS` | Por que a rotina padrão passou a usar `DIVIDE ... REMAINDER` em 07/06/2011, se as três cópias inline mantêm `#SUM - ((#SUM / 11) * 11)`? | `CCVALCPF.NSC:7`, `:99-100`, `:119-120` contra `CADBENEF.NSP:384`, `VALBENEF.NSN:254`, `VALDOCS.NSP:178` | Se a divisão intermediária arredondar em vez de truncar, o resto difere e o dígito verificador calculado também; a mudança de 2011 sugere que o cálculo antigo estava errado em algum caso | Não confirmada: o caso que motivou a correção não está registrado no acervo, e o comportamento depende da precisão intermediária do Natural em produção | Coordenação de Benefícios / DBA Adabas | aberta |
| `BONUS` | Por que `CADDEPEN` valida o CPF do dependente e ignora o resultado ("WARNING MODE")? | `CADDEPEN.NSP:162-171` | Dependentes com CPF inválido entram no grupo periódico; a validação existe mas nunca bloqueia. Mesmo padrão do `SIFAP-M-02` | Não confirmada: o comentário diz aguardar revisão da Coordenação de Benefícios (ticket 6620/2011), a mesma que segue aberta | Coordenação de Benefícios | aberta |
